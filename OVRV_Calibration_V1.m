function Results = OVRV_Calibration_V1(Delta, l, leader, follower, s_real)
    ...
    %parameters of OVRV (Shang et al.), AVs
    k1 = 0.02;     % spacing control gain when accelerating
    k2 = 0.13;     % relative speed control gain when accelerating
    ...
    OVRV_Parameters =  [k1, k2, eta, tau];
    L_bound = [1e-3, 1e-3, 2, 0.1];
    U_bound = [1.0, 1.0, 30.0, 10.0];
    s_min = 0.5;
    ...
    problem = createOptimProblem('fmincon', ...
        ...
        'x0', OVRV_Parameters, ...   % your original guess (used as one of the starts)
        'lb', L_bound, ...
        'ub', U_bound, ...
        'options', Options,...
        'nonlcon', nonlcon);
    ...
    fprintf('\n------------ OVRV Optimal Parameters ------------\n')
    for i = 1:length(solutions)
        fprintf('Run %d: k1=%.3f k2=%.3f eta=%.3f tau=%.3f | error=%.4f\n', ...
            i, solutions(i).X(1), solutions(i).X(2), ...
               solutions(i).X(3), solutions(i).X(4), solutions(i).Fval);
    end
    %}
    
    %% ------------- RMSE CALCULATION -------------
    
    [sim_Opt.x, sim_Opt.v, sim_Opt.a, sim_Opt.s] = OVRV_Dynamics_Loop(Opt_Param, Delta, l, leader.x, leader.v, ...
...
function [c, ceq] = collision_constraint(Parameters, dt, l, x_lead, v_lead, v_follower, s_real, s_min)
    [~,~,~,s_sim] = OVRV_Dynamics_Loop(Parameters, dt, l, x_lead, v_lead, v_follower(1), s_real(1));
